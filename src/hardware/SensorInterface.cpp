...
SensorInterface::SensorInterface(MCP3008* adc, QObject *parent)
    ...
    , m_currentAVL(0.0)
    , m_currentTank(0.0)
    , m_currentClitoral(0.0)
    , m_filteredAVL(0.0)
    , m_filteredTank(0.0)
    , m_filteredClitoral(0.0)
    , m_avlSensorHealthy(true)
    , m_tankSensorHealthy(true)
    , m_clitoralSensorHealthy(true)
    , m_avlErrorCount(0)
    , m_tankErrorCount(0)
    , m_clitoralErrorCount(0)
...
bool SensorInterface::initialize()
{
    ...
    try {
        ...
        double clitoralVoltage = m_adc->readVoltage(CLITORAL_CHANNEL);

        if (avlVoltage < 0 || tankVoltage < 0 || clitoralVoltage < 0) {
            throw std::runtime_error("Failed to read initial sensor values");
        }

        // Initialize filtered values for all three sensors
        m_currentAVL = m_adc->readPressure(AVL_CHANNEL);
        m_currentTank = m_adc->readPressure(TANK_CHANNEL);
        m_currentClitoral = m_adc->readPressure(CLITORAL_CHANNEL);
        m_filteredAVL = m_currentAVL;
        m_filteredTank = m_currentTank;
        m_filteredClitoral = m_currentClitoral;
        ...
        qDebug() << QString("Initial readings - AVL: %1 mmHg, Tank: %2 mmHg, Clitoral: %3 mmHg")
                    .arg(m_currentAVL, 0, 'f', 1)
                    .arg(m_currentTank, 0, 'f', 1)
                    .arg(m_currentClitoral, 0, 'f', 1);
...
double SensorInterface::readClitoralPressure()
{
    if (!m_initialized || !m_adc) {
        return -1.0;  // Error value
    }
    ...
    try {
        double pressure = m_adc->readPressure(CLITORAL_CHANNEL);
        if (pressure >= 0) {
            QMutexLocker locker(&m_dataMutex);
            m_currentClitoral = pressure;

            if (m_filteringEnabled) {
                m_filteredClitoral = applyFilter(pressure, m_filteredClitoral);
            } else {
                m_filteredClitoral = pressure;
            }
        }
        return pressure;

    } catch (const std::exception& e) {
        emit sensorError("Clitoral", QString("Read error: %1").arg(e.what()));
        return -1.0;
    }
}
                                        ...
                                        double zeroPressure, double fullScalePressure)
{
    if (m_adc) {
        ...
        qDebug() << "AVL/Outer chamber sensor calibrated";
    }
}
...
void SensorInterface::calibrateClitoralSensor(double zeroVoltage, double fullScaleVoltage,
                                             double zeroPressure, double fullScalePressure)
{
    if (m_adc) {
        m_adc->calibrateChannel(CLITORAL_CHANNEL, zeroVoltage, fullScaleVoltage,
                               zeroPressure, fullScalePressure);
        qDebug() << "Clitoral cylinder sensor calibrated";
    }
}
...
void SensorInterface::updateReadings()
{
    ...
    double clitoralPressure = readClitoralPressure();

    // Check sensor health
    checkSensorHealth();

    // Emit updated readings if valid (AVL and Tank are essential)
    if (avlPressure >= 0 && tankPressure >= 0) {
        emit pressureUpdated(m_filteredAVL, m_filteredTank);
    }

    // Note: clitoralPressure is read and filtered but not emitted in pressureUpdated
    // The ClitoralOscillator will read it directly via getFilteredClitoralPressure()
    Q_UNUSED(clitoralPressure);
}
...
{
    // Check AVL / Outer chamber sensor
    ...
    // Check Clitoral cylinder sensor
    double clitoralVoltage = m_adc ? m_adc->readVoltage(CLITORAL_CHANNEL) : -1.0;
    if (clitoralVoltage < 0 || !validateReading(clitoralVoltage, "Clitoral")) {
        m_clitoralErrorCount++;
        if (m_clitoralErrorCount >= MAX_CONSECUTIVE_ERRORS && m_clitoralSensorHealthy) {
            m_clitoralSensorHealthy = false;
            emit sensorError("Clitoral", "Sensor unhealthy - too many consecutive errors");
        }
    } else {
        if (m_clitoralErrorCount > 0) {
            m_clitoralErrorCount = std::max(0, m_clitoralErrorCount - 1);  // Gradual recovery
        }
        if (!m_clitoralSensorHealthy && m_clitoralErrorCount == 0) {
            m_clitoralSensorHealthy = true;
            emit sensorRecovered("Clitoral");
        }
    }
}
